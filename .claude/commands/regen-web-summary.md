...
신규 크롤된 `web_sources_raw`를 분류 + ai_summary 작성. `scripts/ai-filter-sources.ts` 호출 (코드 SYSTEM_PROMPT 직접 사용).
...
### 워크플로우
```bash
ANTHROPIC_API_KEY=sk-ant-... bun run scripts/ai-filter-sources.ts [--remote] [--limit N] [--dry-run]
```
