...
| 작업 | 왜 먼저인가 | 완료 조건 |
|---|---|---|
| **네이버 서치어드바이저 등록 확인, 미등록이면 등록** | 이 프로젝트의 성공 판정 지표(4장)가 전부 서치어드바이저에서 나옵니다. 등록 없이는 1단계가 끝나도 결과를 알 수 없습니다. 2026-09-03 검토 시점에 등록 신호를 찾지 못했습니다(**미확인**) | 사이트맵 리포트에 `sitemap-index.xml`이 보이고 유입 검색어 리포트가 열립니다 |
...
| 2026-09-03 (OSM 785건, 로컬 D1 스냅샷 기준) | 785 | **262** | no_evidence 469 · too_few_lots 53 · too_few_lots_with_data 1 | **remote D1 적용 완료 (2026-09-03, 사용자 승인).** destinations 262 · destination_lots 5,541 · evidence 있는 연결 442 · 고아 참조 0. lot_count 분포: 15곳+ 156 · 8~14곳 72 · 5~7곳 27 · 3~4곳 7. 쌍둥이 150쌍 제거. 큰 역(강남·잠실·사당·수원·신촌·판교·부산·대전·반월당·석촌) 전부 통과, 서면역·건대입구역은 no_evidence 탈락 |
