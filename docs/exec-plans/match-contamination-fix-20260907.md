...
## 6-4. Phase 3 실행 결과 — 하류 게이트 통일 (2026-09-07)

### 실측한 현황 (§6 Phase 3 표를 코드로 재확인)

| 위치 | 수정 전 | NULL |
|---|---|---|
| `scoring-engine.ts:64,73` | `= 1` | 제외 |
| `destinations.ts:92` | `= 1` | 제외 |
| `lot-summary-batch.ts:170`, `generate-lot-summary.ts:132` | `IS NOT 0` | 포함 |
| `parking.ts:339` (블로그 노출) | `IS NULL OR != 0` | 포함 |
| **`sitemap-handler.ts`** | **`filter_v2_reason IS NOT 'aggregator_site'`** | 게이트 아님 |
| **`wiki/index.tsx`** | **없음** | 게이트 없음 |

사이트맵이 **사유 문자열 하나만** 보고 있었다. `filter_passed_v2=0` 이어도 사유가
`aggregator_site` 가 아니면 그대로 통과한다 — #175 가 요약 입력 경로에서 고친 것과
**똑같은 버그**가 사이트맵에 남아 있었다. Phase 2 에서 `relevance_score` 까지 함께
내려야 했던 이유가 이것이다.

### 정책: `IS NOT 0` (알려진 불량만 제외)

`= 1` 을 기본으로 삼지 않는 이유는 `match-to-lots.ts:495-508` 이 이미 적어둔 그대로다 —
**`1` 의 뜻이 경로마다 다르다.** FILTER_V2 를 거친 행의 `1` 은 "이 주차장 글이 맞다",
rule=high 행의 `1` 은 "글이 괜찮다"일 뿐이다. 게다가 `= 1` 은 미판정(NULL)까지 버린다.
확실한 것은 **`0` 은 우리가 불량이라고 판정한 것**뿐이므로, 그것만 제외한다.

### 적용

| 파일 | 변경 | 영향 |
|---|---|---|
| `sitemap-handler.ts` (4곳) | 사유 문자열 → `filter_passed_v2 IS NOT 0` | 사이트맵 lot **20,920 → 20,253** (-667, 3.2%) |
| `wiki/index.tsx` (3곳) | 게이트 신규 추가 | 목록·`web_count` 가 탈락 근거를 안 셈 |

두 쿼리 모두 remote D1 에서 직접 실행해 문법·결과를 확인했다. sub-sitemap 개수는 5개로 동일.

### 의도적으로 남겨둔 것 — 스코어링 입력의 `= 1`

`scoring-engine.ts` 는 `= 1` 로 뒀다. `IS NOT 0` 으로 바꾸면 미판정 **3,002행·1,445곳**이
점수 산정에 들어와 **이용자에게 보이는 순위가 바뀐다.** 점수 재계산과 분포 비교가
선행돼야 하는 별도 판단이므로 Phase 3(게이트 의미 통일)의 범위 밖으로 두고,
코드에 이유를 주석으로 남겼다.

`destinations.ts` 의 `= 1` 도 그대로다 — #166 목적지 페이지가 대기 상태라 지금 건드릴
이유가 없다.
