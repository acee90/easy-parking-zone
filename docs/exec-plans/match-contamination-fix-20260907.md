...
Phase 1 은 **손실 기준은 지켰고 차단 기준은 못 지켰다.** 다만 미달의 원인이 "설계가 틀렸다"가 아니라 "겨냥한 표적이 오염의 21%뿐이었다"이므로, 되돌리지 않고 **Phase 2 를 wrong_lot 대응으로 확장**하는 것이 맞다.

1. **Phase 2 (정리)** — 계획대로 진행하되, 대상 산정에 새 채점을 그대로 쓴다(14곳 표본에서 56% 제거).
2. **Phase 2.5 (신규 · wrong_lot 대응)** — `pipeline-149` §3 Phase 2 의 미완 항목:
   - **최적 lot 중재**: 임계값 넘은 후보를 전부 INSERT 하지 말고 최고점 1곳만(동점이면 AI).
