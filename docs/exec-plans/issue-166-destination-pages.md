...
| 확인할 항목 | 통과 기준 |
|---|---|
| 네이버 서치어드바이저 등록 여부 | 등록되어 있어야 합니다. 2026-09-03 검토 시점에 DNS TXT, `/naver-site-verification.html`, 홈 `<meta>` 어디에도 등록 신호가 없었습니다(**미확인**, 사람이 로그인해서 확인해야 합니다). 등록되어 있지 않다면 이것이 첫 번째 작업입니다 |
