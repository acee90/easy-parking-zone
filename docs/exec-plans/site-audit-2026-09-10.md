...
| 5 | web_sources 0건 주차장 수 | `SELECT COUNT(*) FROM parking_lots p LEFT JOIN (SELECT DISTINCT parking_lot_id id FROM web_sources) w ON w.id=p.id WHERE w.id IS NULL` | **43,172** | **1주 뒤 −3,000 이상** (600곳/일 × 7일 중 매칭 성공분) | +7일 | **중간값 (09-11, 약 21시간): 크롤로 첫 근거를 얻은 lot 64곳 → 1주 환산 약 450곳. 목표 미달 예상.** 배포 후 naver 가 크롤한 497곳 중 근거가 붙은 곳은 52곳(약 10%). 파이프라인 적체는 없다(본문 대기 0, 필터 대기 69, 매칭 대기 100) — **병목은 처리량이 아니라 수율**이다. 근거 0건 lot 은 대부분 블로그에 거의 안 나오는 소규모 공영 주차장이다. 기대값을 「크롤한 lot 대부분에 근거가 붙는다」는 가정으로 세운 게 틀렸다. **중간 2 (09-14): naver 가 09-10 배포 후 크롤한 lot 2,247곳 중 근거가 붙은 곳 154곳 (6.9%).** A-2 고정 763곳을 빼면 1,484곳 중 119곳 (8.0%) — 09-11 검수자 측정 8.6% 와 비슷. 병합 대표 lot 은 빼지 않았다. 최종 판정 09-17 |
...
| 2′ | **09-14 전체 재측정** (763곳 × 크롤러) | 고정 / 크롤 후 해제 / **도래했는데 해제(오류)** | — | 오류 0 | +3일 | **✅ 오류 0.** naver 763곳·ddg 763곳 전부 크롤 후 해제(#206 규칙대로), youtube 159곳 크롤·604곳 고정 유지(BATCH_SIZE 4 라 느림). brave_search 763곳은 전부 고정 — **brave 크롤러가 09-05 이후 한 건도 기록이 없다**(`crawl:brave` 0행, 큐 next_at 최대 09-10). A-2 문제가 아니라 기존 크롤러 정지. 별도 작업으로 뺐다 |
| 3 | 대상 lot 중 web_sources ≥1 비율 | web_sources 조인 | 0% (PV 있는데 근거 0) | **≥ 60%** | +7일 | **중간 (09-14): 35/763 (4.6%).** 대조군(같은 기간 크롤된 고정 외 p0) 8.0% 보다 **낮다** — 고정분이 먼저(09-11) 크롤돼 파이프라인 대기 시간은 오히려 더 길었는데도. 「조회되는 lot 은 블로그에도 더 나온다」는 가설이 지금까지는 맞지 않는다. 최종 09-18 |
...
|---|---|---|---|---|---|---|
| 1 | `web_sources_missed` 신규 행 | `created_at > 배포 시각` | **0건/일** (08-19 이후 0) | **≥ 30건/일** | +1일 | 중간값 (09-11 06:00 크론 1회, 매칭 raw 약 14건): **0건.** 크론 raw 는 특정 주차장을 검색해 모은 글이라 FTS 후보 0건이 드물다. 30건/일 기대값은 수동 파이프라인(대량 일괄) 기준이라 과할 수 있다 — 09-12 하루치로 판정. **최종 (09-14 확인): ✗ 09-11 3건 / 09-12 5건 / 09-13 3건.** 기대값 ≥30건/일에 크게 못 미친다. 크론 raw 는 특정 주차장을 검색해 모은 글이라 FTS 후보 0건 자체가 드물다 — 기대값을 수동 일괄 파이프라인 기준으로 잘못 세웠다 |
| 2 | ~~신규 raw 중 `match_fail_reason` 기록 비율~~ → **`pipeline_daily_stats` 의 `match:missed` / `match:noise_name`** | 일별 카운터 | 0 | 후보 0건 raw 가 생기면 두 카운터 합 = 그 수 | +1일 | **측정 설계 수정.** 매칭 끝난 raw 는 같은 크론의 purge 가 곧바로 지워 `match_fail_reason` 을 사후에 볼 수 없다(09-11 실측: 매칭 raw 전부 삭제됨). 매처가 두 카운터를 반환하고 크론이 일일 통계에 남기도록 고쳤다. **09-14 확인 ✅** `match:missed` 3·5·3 = missed 신규 행 3·5·3 과 일치, `match:noise_name` 09-12 3건 (09-11~13) |
| 3 | 신규 missed 상위 20 이름 중 실제 장소명 비율 | 수동 검수 | — | **≥ 50%**. 미만이면 노이즈 규칙 보강 | +7일 | **중간 (09-14, 전수 11건 — 표본 부족, 자기 판정): 실제 장소명으로 읽히는 것 약 4/11** (민머루해수욕장, 태화강국가정원4공영, 서울아트책보고, 울산체육공원). 나머지는 블로그 제목 조각(「불당맛집 남해바다 멸치국수 3탄」「10과제 최태환」). 노이즈 규칙이 제목 조각을 못 거른다 — 행이 적어 missed 표 오염 규모는 작다 |
| 4 | 매칭 단계 처리량 | `pipeline_daily_stats` match:* | 현행 | **변동 ≤ 10%** (DB 쓰기 1건 추가뿐) | +1일 | **✅ 감소 없음** — `match:sources` 09-10 35 → 09-11 62 / 12일 35 / 13일 50, `match:links` 65 → 113 / 121 / 147. (±10% 는 일변동이 커서 판정 불가 — 독립 검수 지적대로 감소 여부만 본다) |
...
| 2 | **✅ 후보 250개 → rows_read 748** (z14·전역 동일, 독립 검수 재현 828 — 측정 좌표를 안 남겨 조건이 조금 달랐다). 기준선 1,314 / 738 이하. 필터가 걸리면 후보 600 → 약 1,250~1,790 |
| 3 | **조회 횟수 ✅ 1회 / 첫 목록 표시 △** (운영, 2회 재측정). 횟수만 보면 통과지만 목록이 포인트를 기다리게 돼 첫 목록 표시가 늦어질 수 있다 (독립 검수 지적 — B-3 #5 에서 콜드/웜 분리 측정). #211 배포 직후에는 여전히 2회였다 — 포인트(1.29s 걸림) 도착 전 첫 bounds 이벤트가 폴백 GET(2.21s)을 보내고, 포인트가 오면 최근접 POST(2.95s)로 다시 바꿨다. **#212** 에서 포인트가 오는 중이면 폴백하지 않고 기다리게 했다. 재측정: 포인트 0.58→1.54s, 목록 POST 1.79→2.24s 1회 / 포인트 0.90→2.44s, 목록 POST 3.45→3.92s 1회. GET 폴백 0회. 대가는 포인트가 늦으면 목록도 그만큼 늦는다는 것 — B-3(포인트 경량화)이 이 시간을 줄인다 |
...
| 5 | 첫 목록 **도착** 시각 (B-2 가 포인트를 기다린다) | B-2 이전 첫 GET 약 2.0s (운영 1회, 캐시 헤더 없던 시절) | 늘지 않음 | **재방문 ✅ 중앙 1.40s** (1.23~2.06s, n=5, 포인트는 브라우저 캐시). **캐시 없는 첫 방문 △ 미확정** — 배포 직후 1회 4.04s 시작(엣지·Cache API 모두 빈 상태). 포인트 단독 no-store fetch 는 0.24~0.39s (n=5) 라 캐시가 찬 뒤의 첫 방문은 재방문 + 약 0.3s 로 추정. 브라우저 캐시를 비울 수 없어 콜드 n≥5 는 못 쟀다. 기준선과 콜드끼리 비교한 값이 아니다 (독립 검수 지적) |
...
- **D-2 #6 의 109 → 38 은 B-3 효과와 섞였다** (독립 검수). B-3 이전에는 목록 밖 lot 의 경량 마커에도 이름 라벨이 있었는데, 지금은 이름이 없어 점수와 무관하게 점이다. 라벨 감소 중 D-2 규칙(목록 안 점수 없는 lot → 점) 몫과 B-3·B-1(250개 상한) 몫을 나눠 세지 못했다. 반영 후 라벨 38 + 점 87 = 125 로 기준선 마커 109 와 장면 구성도 같지 않다. 판정은 「B-3·D-2 합산 효과」로 읽을 것. 목록 밖 점은 클릭·hover 가 없다(원래부터) — z16 에서 이런 점이 늘어난 것은 반대 지표로 관측할 것.
...
**운영 재측정 (09-11, #213 이후 코드 — 독립 검수 지적: 위 값은 B-1·B-2·B-3 이전 로컬 n=1)**

1280×800, 서울 기본 위치, 목록 2·4·6·8·10번째 lot 을 차례로 클릭(n=5)하고 스크립트로 기록.

| # | 결과 |
|---|---|
| 1 | **✅ 클릭 → 상세 열림 27~58ms** (5/5) |
| 3 | **△ 클릭 → 목록 응답 도착 1.08~1.14s** — 계획의 검증 기준 「이동 → 목록 갱신까지 1초 이내」 근소 미달. 내역: panTo 250ms + bounds 디바운스 350ms + id 조회 약 0.5s. 진행 기록의 「지도 이동 250ms」는 **코드 상수**다(측정값 아님). 계획의 `morph` 대신 `setZoom(16)` 후 250ms `panTo` 라 줌은 애니메이션 없이 먼저 바뀐다 — 체감은 평가하지 않았다 |
| 4 | **✅ 클릭당 목록 조회 1회** (5/5, POST id 조회) |
| 반대 지표 | **✅ 상세를 닫으면 방금 고른 행이 목록에 보인다 5/5** |

**C-2 후속 — idle 즉시 조회 (#216, 09-14 00:24 UTC 배포)**

09-14 운영 기준선(n=5)으로 1초 초과를 쪼갰다: 클릭 → 조회 시작 **646~660ms** + 서버 TTFB **316~452ms** = 978~1,101ms. 조회 시작까지는 panTo(250ms) 뒤 애니메이션 중 마지막 `bounds_changed` 부터 `PAN_SETTLE_MS`(350ms)를 다시 기다린 몫이다. 서버 쪽은 같은 250개 id 쿼리의 D1 SQL 이 **6~9ms**(rows_read 829, ICN) — TTFB 대부분은 측정 브라우저(LAX 콜로)↔ICN 왕복이라 코드로 줄일 몫이 아니다. 계획의 「1초 이내」는 측정 위치를 정하지 않고 세운 목표였다.

변경: 클릭(목록·마커·클러스터)으로 옮길 때는 지도 `idle` 이벤트 50ms 뒤 bounds 를 보낸다 (`setZoom`·`panTo` 가 idle 을 두 번 낼 수 있어 묶는다). 사용자 드래그는 기존 300ms 디바운스 그대로.

| # | 지표 | 기준선 (09-14 운영 n=5) | 기대값 | 결과 (운영 n=5) |
|---|---|---|---|---|
| 1 | 클릭 → 목록 조회 시작 | 646~660ms | ≈ 300ms 대 | **✅ 355~420ms** (데스크톱), **324~328ms** (모바일 375×812) |
| 2 | 클릭 → 목록 응답 도착 | 978~1,101ms | ≤ 1초 | **△ 757~1,497ms, 중앙 877ms** (데스크톱). 4/5 가 1초 이내, 1건은 서버 TTFB 1,133ms 튐 — LAX↔ICN 왕복 편차 |
| 반대 지표 | 클릭당 목록 조회 수 | 1 | 1 | **✅ 1/5 데스크톱, 1/5 모바일** (idle 중복 발생 없음) |
| 반대 지표 | 사용자 드래그 조회 수 | 1 | 1 | **✅ 1** (로컬, 드래그 후 359ms) |
| 반대 지표 | 모바일 하단 시트 보정 | 마커가 시트 위 | 불변 | **✅ 5/5** — 카드 열림, 선택 마커 하단 294px < 시트 상단 492px |

- 줌이 애니메이션 없이 먼저 바뀌는 점(`setZoom(16)` 후 `panTo`)은 그대로다. `morph` 로 바꾸려면 모바일 시트 보정을 도착 줌 기준으로 다시 계산해야 해 범위 밖으로 남긴다.
...
- **의존**: A-6 과 자리 공유.
- **결과 (09-11)**: **✅ 추가 변경 없음 — A-6(#204)에서 끝났다.** 운영 지도 첫 화면 목록 20행에서 「데이터 없음」 0건 (시청 주변 20곳이 모두 점수 보유면 저절로 참이 되는 약한 증거다 — 실제 근거는 아래 코드 가드). 모바일 패널은 `score !== null` 일 때만 라벨을 그리고, 목적지 페이지 `EasiestPicks` 는 점수 없는 lot 을 걸러낸다. `ParkingCard` 에는 이 배지가 원래 없다 (점수 칸은 「- / 5」).
...
측정: 경향신문 주차장(MODU-109427), 기준선은 운영, 반영 후는 로컬 dev(같은 리모트 D1). **#213 배포 후 운영에서 같은 값을 다시 확인했다** (빈 문구 0, 별점 783px, 패널 스크롤 높이 746px, 데스크톱). 한 lot, n=1.
...
| 반대 지표 | 개수 조회 실패 시 | — | 글이 있는 lot 을 「없어요」로 접지 않음 | **✗ → 수정.** 처음 구현은 실패해도 `.finally` 에서 준비 완료가 돼 0개로 접혔다 (독립 검수 지적). 실패하면 `countsReady` 를 넘기지 않아 예전처럼 세 섹션을 그리도록 고쳤다 |

- #1 「빈 문구 0」은 변경 그 자체라 정의상 참이다. 효과 지표는 #2~#4 다.
...
| 4 | 반대 지표: 규칙이 막는 lot 과 오탐 | — | 오탐(적합한데 막힘) ≤ 10% | **△ 점추정 3.5% (제외 규모 가중), 보수적 상한 18.4%.** 3차(새 채점자, 배포 규칙)로 가장 큰 두 버킷을 30개씩 보강 — 행정구역명만(1,840곳) 1/40·95% 상한 13%, 구간·도로(1,563곳) 0/40·상한 9%. 제외의 74% 를 차지하는 두 버킷은 좁혀졌지만 작은 버킷(일반명·시군구명·출입구 등 1,145곳)은 1/10씩이라 상한 40% — 「≤10%」는 입증하지 못했다. 대조군 15개 중 1개를 채점자가 부적합으로 봄(「울주군 사유지 개방주차장」). 2차는 3/50 — 버킷 5개 × 10개 층화. 0/10 의 95% 상한이 약 26% 라 「≤10%」를 입증하지 못하고, 버킷 크기가 최대 35배 차이 나는데 가중하지 않았다 (독립 검수 지적). 채점한 after 목록은 오탐 2건 수정 **이전** 규칙이다 (배포 규칙은 제외가 더 적다). 전체 53,868곳 중 **4,548곳(8.4%)** 제외 — 행정구역명만 1,840 / 구간·범위 표기 1,456 / 일반명 764 / 시군구명 186 / 대행·세차·차고지 140 / 도로 103 / 입구·출구 53 / 기타 6 |
...
- **결과 (09-11, 스타필드시티 위례 주차장 KA-1935812519, 운영 → 로컬 dev)**
  ...
  - 1시간 요금 칸(`HourFee` — 계획에 적은 `AlternativeLotsSection` 이 아니라 `RelatedParkingLotsSection.tsx`, 위키 비교표·목적지 페이지 공용): 「확인 필요」 → 「—」(스크린리더에는 `sr-only` 「요금 정보 없음」) → **✅ 보이는 「확인 필요」 1 → 0**
...
| C-1·C-2·D-6 | #210 (09-11) + #216 (09-14) | **완료 (C-2 △)** | #216 idle 즉시 조회 후 운영 n=5: 조회 시작 646~660 → 355~420ms, 목록 응답 중앙 877ms(4/5 가 1초 이내, 나머지는 LAX↔ICN 왕복 편차), 클릭당 조회 1회·모바일 시트 보정 유지. 이전 운영 재측정: 클릭 → 상세 27~58ms, 클릭 → 목록 응답 1.08~1.14s(목표 1초). 지도 이동 250ms 는 코드 상수. 로컬 첫 측정: 목록 1회 클릭 74ms 뒤 상세, 클릭당 목록 조회 1회, 목록 교체 시 20행 리셋 |
| B-1·B-2 | #211 + #212 (09-11 07:41 UTC) | **완료** | 첫 행 = 실제 최근접(운영 17m, 기준선 2.8km), rows_read 748(기준선 1,314/738), 첫 로딩 목록 조회 2회 → 1회. 첫 목록 표시는 재방문 중앙 1.40s, 캐시 없는 첫 방문 미확정 △ |
