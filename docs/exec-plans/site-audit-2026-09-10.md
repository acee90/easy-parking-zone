...
| # | 지표 | 기준선 (09-11 운영) | 기대값 | 결과 (로컬 dev → 운영은 배포 후) |
|---|---|---|---|---|
| 1 | 포인트 응답 해제 크기 | 11.5MB | ≤ 3MB | **✅ 2.14MB (−81%)** |
| 2 | 포인트 응답 전송 크기 | 1.86MB | ≤ 600KB | gzip 추정 **579KB** (브라우저 CompressionStream). 운영 실측 대기 |
| 3 | 포인트 응답 시간 | 1.84s (TTFB 1.12s) | 감소 | 운영 실측 대기 |
| 4 | 지도 1회 방문 포인트 rows_read (반대 지표) | 캐시 적중에도 53,868 | 캐시 적중 시 0 | 코드상 0 (버전 쿼리 삭제). 운영 확인 대기 |
| 5 | 첫 목록 조회 시각 (B-2 가 포인트를 기다린다) | 1.79~3.45s | 감소 | 운영 실측 대기 |
| 6 | z16 서울 중구 지도 영역 이름 라벨 | **109** (점수 없음 76) | 점수 있는 곳 + 선택 lot 만 | **✅ 38** (보통 35, 별로 2, 선택 lot 1). 점수 없는 lot 은 회색 점 87 |
...
| D-4 | `feat/b3-points-api` 에 포함 | **블라인드 채점 완료, 배포 대기** | 2차(최종): 부적합 44 → 11/357 (3.1%, 목표 ≤2% 미달), 중복 3 → 0, 제외 오탐 3/50. 남은 부류는 동·읍·면명 + 주차장 |
| D-3 | `feat/d4-curation-quality` | **로컬 검증, 배포 대기** | 빈 문구 3 → 0, 패널 높이 −33%, 별점 879 → 783px(데스크톱은 여전히 약 32px 스크롤 — 부분 달성) |
| B-3·D-2 | `feat/b3-points-api` | **로컬 검증, 배포 대기** | 포인트 해제 11.5MB → 2.14MB, gzip 579KB, 버전 쿼리(5.4만 rows_read/방문) 삭제, z16 라벨 109 → 38 |
| D-5 | `feat/b3-points-api` | **로컬 검증, 배포 대기** | 「혜택CGV」 → 「혜택 CGV」, 비교표 「확인 필요」 → 「—」 |
