...
### A-3. YouTube search.list 우회

- **목표**: 100유닛짜리 search.list를 없애고 이미 가진 URL에서 1유닛 videos.list로 메타 수집. 하루 48곳 → 수천 건.
- **변경**
  - `scripts/harvest-youtube-from-raw.ts`: `web_sources_raw.source_url` 에서 `youtube.com/watch?v=` / `youtu.be/` 추출 → video id 목록(1,734건 시작점) → `videos.list`(id 50개씩 배치, 1유닛/콜) → 기존 youtube 크롤러의 저장 경로 재사용(`parking_media` + `web_sources source='youtube_*'`).
  - `src/server/crawlers/youtube.ts`: search.list 경로는 남기되 BATCH_SIZE를 4 → 2로 줄이고, 절약된 쿼터를 videos.list 배치 스테이지에 배정. 크론에 스테이지 추가는 `scheduled.ts` 메인 파이프라인 안에서 subrequest 예산 확인 후.
  - 이후 발견 경로: ddg / naver 검색 쿼리에 `site:youtube.com` 변형 1개 추가 (`naver-blogs.ts` 쿼리 빌더). 별도 PR로 분리 가능.

#### A-3 평가항목

| # | 지표 | 측정 | 기준선 (09-10) | 기대값 | 시점 | 결과 |
|---|---|---|---|---|---|---|
| 1 | raw 에서 추출한 고유 video id | 스크립트 출력 | raw 1,734건 | **≥ 1,000 고유 id** | 스크립트 직후 | |
| 2 | videos.list 쿼터 소모 | 콜 수 × 1유닛 (50개/콜) | search 9,600/일 | **≤ 50유닛** (1,000건 기준 20콜) | 스크립트 직후 | |
| 3 | `parking_media` 증가분 | 전후 COUNT | 현행 | **≥ 500건** | 스크립트 직후 | |
| 4 | 매칭 성공률 (video → lot) | 저장분 ÷ 시도분 | — | **≥ 40%**. 미만이면 매칭 규칙 재검토 | 스크립트 직후 | |
| 5 | YouTube 일 쿼터 사용량 | GCP 콘솔 | 9,600 / 10,000 | **≤ 5,000** (BATCH_SIZE 4→2) | +1일 | |

- **판정**: 1~4 즉시 판정. 4가 40% 미만이면 **저장하지 말고** 규칙 수정 후 재실행 (정확도 우선).
