...
| A-3 | | | |
| A-4 | #201 (09-11) | **완료 (정정)** | 207쌍 병합 → 독립 전수 재검수 98.6% → **4쌍 되돌림, 순 203쌍**. web_sources 손실 0, 대표 재계산 105/105 |
...
- A-6: `user_reviews` 에 입력 경로(`entry_point`: form / inline / revisit) 컬럼 추가 — 마이그레이션 + `createReview` 수정
- A-6: 서버 rate limit 을 IP+lot 에서 「IP 당 시간당 N곳」으로 넓힐지
