...
후보 수는 필터가 없으면 250(평소 비용이 현재 이하), 필터가 있으면 600 으로 둔다. 포인트가 아직 안 불러와졌으면 지금의 bounds 조회로 폴백한다. id 600개는 URL 로 약 8.4KB 라 POST 로 보낸다.
...
| 3 | 운영 배포 후 측정 (로컬은 리소스 타이밍 버퍼 초과) |
| 4 | 운영 배포 후 측정 |
| 필터 | 「무료만」 → 첫 20행 중 유료 0, 목록 조회는 POST(id 조회). 토글 1회에 조회 2회가 관측됨 — 기준선과 비교 필요 |
