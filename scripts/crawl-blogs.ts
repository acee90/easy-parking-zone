...
const LOT_IDS = (() => {
  const flag = process.argv.find((a) => a.startsWith('--lot-ids='))
  if (!flag) return null
  return flag
    .split('=')[1]
    .split(',')
...
const progressSuffix = LOT_IDS
  ? `${engineName}-targeted`
  : UNCOVERED_ONLY
    ? `${engineName}-uncovered`
    : engineName
...
/** 검색 결과 관련도 점수 (0-100) */
function scoreRelevance(item: SearchItem, name: string, address: string): number {
  let score = 0
  ...
  if (titleMatches.length >= 2)
    score += 50 // 키워드 2개 이상 제목 매칭 → 강한 신호
  else if (titleSpecificMatches.length >= 1)
    score += 40 // 고유 키워드 1개 제목 매칭
...
async function main() {
  ...
  if (LOT_IDS) console.log(`🎯 타겟 ${LOT_IDS.length}개 lot`)
  else if (UNCOVERED_ONLY) console.log('🎯 미커버 주차장만 대상')
  console.log()
