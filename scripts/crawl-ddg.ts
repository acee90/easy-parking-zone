...
const LOT_IDS = (() => {
  const flag = args.find((a) => a.startsWith('--lot-ids='))
  if (!flag) return null
  return flag
    .split('=')[1]
    .split(',')
...
function parseDdgHtml(html: string): DdgResult[] {
  ...
  if (results.length === 0) {
    ...
    const snippetFb = /<a[^>]+class="result__snippet"[^>]*>([\s\S]*?)<\/a>/gi
...
function flushResults(
  ...
  for (const r of pending) {
    stmts.push(
      buildInsert('web_sources_raw', RAW_COLS, ['ddg_search', r.sourceId, r.url, r.title, r.desc]),
    )
  }
...
async function main() {
  ...
  for (let i = 0; i < lots.length; i++) {
    ...
    if (!isDryRun)
      process.stdout.write(`→ ${lotSaved}건 (${queries.map((q) => q.strategy).join('+')})\n`)
