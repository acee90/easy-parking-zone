...
const SUPERCLUSTER_OPTIONS: Supercluster.Options<PointProperties, ClusterProperties> = {
  radius: 200,
  maxZoom: 15,
  minZoom: 0,
  map: (props) => ({
...
export function useSuperCluster(points: ParkingPoint[] | null) {
  ...
  const index = useMemo(() => {
    ...
    const features: Supercluster.PointFeature<PointProperties>[] = points.map((p) => ({
