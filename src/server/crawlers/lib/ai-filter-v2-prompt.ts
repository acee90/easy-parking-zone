...
export const FILTER_V2_SYSTEM_PROMPT = `주차장 풀텍스트 본문을 분석하여 SEO 가치 있는 후기만 통과시키는 JSON 분류기입니다.
...
- "thin": 본문 200자 미만 또는 주차장 자체에 대한 구체 정보 부재
- "wrong_lot": 본문에 입력으로 받은 lot 이름 (또는 핵심 키워드) 이 한 번도 등장하지 않음 → 다른 주차장 얘기
- "boilerplate": SEO 자동생성 템플릿 ("Top5 저렴한 주변 주차정보", 운영시간/요금만 나열, 공식 가이드 톤)
...
- 본문에 lot 이름 명시 + 진짜 사용자 후기 (방문 경험, 진입로/주차면/요금 구체 묘사) 1건 이상
...
- 본문이 여러 주차장을 나열한다 해도, 입력 lot 이름 등장 부분이 있으면 통과 가능.
