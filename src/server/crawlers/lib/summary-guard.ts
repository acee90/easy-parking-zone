...
const CHROME_PATTERNS: { name: string; re: RegExp }[] = [
  ...
  // 문서 자기소개로 시작하는 요약. 주차장 요약은 주차장 얘기로 시작해야 한다.
  // 2026-09-03 실측에서 여러 주차장을 나열한 집계 글이 boilerplate 판정을 빠져나가
  // "본 문서는 … 가이드입니다" 형태로 저장되려 했다. 기존 8,700건에는 0건이라
  // 오탐 없이 추가할 수 있다.
  { name: 'doc_meta_opener', re: /^\s*(?:본|이)\s*(?:문서|글|포스팅|페이지)[는은]/ },
  // 프롬프트 지시문을 그대로 되뱉은 것. 실측 9건이 이 상태로 노출 중이었다.
  { name: 'prompt_echo', re: /재작성한 주차 가이드|200~600자|120~180자/ },
]
