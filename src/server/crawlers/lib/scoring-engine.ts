...
function computeSourceScores(reviews: ReviewRow[], texts: TextRow[], now: Date) {
  ...
  const communityScore = weightedAvg(
    communityReviews.map((r) => ({
      score: r.overall_score, date: r.created_at, weight: r.is_seed === 1 ? 0.3 : 0.6,
    })),
    now,
  );
...
export async function recomputeStats(
  ...
  // 대상 주차장 정보
  const lots = await db
    .prepare(`SELECT id, name, type, total_spaces, is_free, notes FROM parking_lots WHERE id IN (${placeholders})`)
    .bind(...lotIds)
    .all<ParkingLotRow>();
  ...
  const reviews = await db
    .prepare(`SELECT parking_lot_id, overall_score, is_seed, source_type, created_at FROM user_reviews WHERE parking_lot_id IN (${placeholders})`)
    .bind(...lotIds)
  ...
  for (const lot of lots.results) {
    ...
    batch.push(
      db.prepare(
        ...
        lot.id, prior,
        sources.userReviewScore, sources.userReviewCount,
        sources.communityScore, sources.communityCount,
        sources.textScore, sources.textCount,
        sources.nEffective, finalScore, reliability,
      ),
    );
  }
