...
async function searchDuckDuckGo(query: string, crawl4aiUrl: string): Promise<DdgResult[]> {
    ...
    for (const cq of queries) {
      try {
        ...
        if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
          console.log(
            `[ddg-search] ${MAX_CONSECUTIVE_FAILURES} consecutive failures, stopping batch`,
          )
          break
        }
      }
