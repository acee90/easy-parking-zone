...
async function searchVideos(query: string, maxResults: number, apiKey: string) {
  const params = new URLSearchParams({
    ...
    maxResults: String(maxResults), key: apiKey, relevanceLanguage: "ko",
  });
...
async function getComments(videoId: string, maxResults: number, apiKey: string) {
  const params = new URLSearchParams({
    part: "snippet", videoId, maxResults: String(maxResults),
...
export async function runYoutubeBatch(
  ...
  const progress = await db
    .prepare("SELECT last_parking_lot_id, completed_count FROM crawl_progress WHERE crawler_id = 'youtube'")
    ...
    for (const video of videos) {
      ...
      batch.push(
        db.prepare(
          ...
          lot.id, videoUrl, video.snippet.title,
      ...
      // 댓글 수집
      try {
        const comments = await getComments(video.id.videoId, COMMENTS_PER_VIDEO, env.YOUTUBE_API_KEY);
