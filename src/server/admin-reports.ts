...
/** 신고 통계 */
export const fetchReportStats = createServerFn({ method: "GET" }).handler(
  async ({ request }) => {
    if (!request) throw new Error("서버 요청 필요");
    await requireAdmin(request);
  ...
  .handler(async ({ data, request }) => {
    ...
    if (data.action === "resolve") {
      if (report.targetType === "web_source") {
        ...
        await db
          .delete(schema.parkingMedia)
          .where(eq(schema.parkingMedia.id, report.targetId));
      } else if (report.targetType === "review") {
        await db
          .delete(schema.userReviews)
          .where(eq(schema.userReviews.id, report.targetId));
      }
    }
