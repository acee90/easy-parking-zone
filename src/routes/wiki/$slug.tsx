...
import type { BlogPost, NearbyPlaceInfo } from "@/types/parking";

export const Route = createFileRoute("/wiki/$slug")({
	loader: async ({ params }) => {
		const id = parseIdFromSlug(params.slug);
		if (!id) throw notFound();
		const [lot, nearbyPlaces, topSnippets] = await Promise.all([
			fetchParkingDetail({ data: { id } }),
			fetchNearbyPlaces({ data: { parkingLotId: id } }),
			fetchBlogPosts({ data: { parkingLotId: id, limit: 5 } }),
		]);
		if (!lot) throw notFound();
		return { lot, nearbyPlaces, topSnippets };
	},
	head: ({ loaderData }) => {
		...
		return {
			...
			links: [
				{ rel: "canonical", href: `https://easy-parking.xyz/wiki/${slug}` },
			],
			headScripts: [
				{
...
const SOURCE_LABELS_SSR: Record<string, string> = {
	naver_blog: "네이버 블로그",
	naver_cafe: "네이버 카페",
	clien: "클리앙",
	youtube_comment: "유튜브",
...
function BlogSnippetsSection({ snippets }: { snippets: BlogPost[] }) {
	if (snippets.length === 0) return null;
	return (
		<section className="space-y-3 border-t pt-4">
			<h2 className="font-semibold text-base">방문 후기</h2>
			<div className="space-y-3">
				{snippets.map((post) => (
					<div
						key={post.id}
						className="rounded-lg border px-3 py-2.5 bg-gray-50"
					>
						...
						<p className="text-xs text-muted-foreground mt-1.5">
							{SOURCE_LABELS_SSR[post.source] ?? post.source}
...
function WikiDetailPage() {
	const { lot, nearbyPlaces, topSnippets } = Route.useLoaderData();
			...
			<div className="max-w-6xl mx-auto px-4 py-6">
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
					...
					<div className="md:col-span-2 space-y-4">
						...
						{/* 방문 후기 (SSR — 검색엔진 노출용) */}
						<BlogSnippetsSection snippets={topSnippets} />
