...
export const Route = createFileRoute('/admin/web-sources')({
  loader: async () => {
    const [signalData, signalStats, wsData, wsStats, unmatchedData, unmatchedStats] =
      await Promise.all([
      ...
      <div className="flex gap-1 mb-6 border-b">
        {(
          [
            ...
            [
              'unmatched',
              `POI 매칭 실패${unmatchedStats.pending > 0 ? ` (${unmatchedStats.pending})` : ''}`,
            ],
          ] as const
        ).map(([value, label]) => (
          <button
...
function SignalsSection() {
  ...
  const [statusFilter, setStatusFilter] = useState<'pending' | 'tagged' | 'irrelevant' | 'all'>(
    'pending',
  )
  ...
  async function handleTag(signalId: number, score: number | null) {
    ...
    if (statusFilter === 'pending' && score !== null) {
      ...
      setSignals((prev) => prev.map((s) => (s.id === signalId ? { ...s, humanScore: score } : s)))
    }
  ...
  async function handleRemoveLot(signalId: number, lotId: string) {
    ...
    setSignals((prev) =>
      prev.map((s) =>
        s.id === signalId ? { ...s, lots: s.lots.filter((l) => l.parkingLotId !== lotId) } : s,
      ),
    )
  }
        ...
        <div className="flex justify-center gap-2 mt-4">
          <button
            onClick={() => {
              ...
              setPage(p)
              reload(statusFilter, lotSearch, p)
            }}
            disabled={page === 1}
        ...
        <div className="bg-white rounded-lg border overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((r) => {
                  const badge = WS_SOURCE_BADGE[r.source] ?? {
                    label: r.source,
                  ...
                  return (
                    <tr key={r.id} className="border-b hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3 text-xs font-medium truncate max-w-[160px]">
                        {r.parkingLotName}
                      </td>
...
function StatCard({
  ...
  const colorClass = color === 'green' ? 'text-green-600' : color === 'gray' ? 'text-gray-400' : ''
...
const SCORE_BUTTONS = [
  ...
  { score: 2, label: '💀', title: '어려움', style: 'bg-red-50 text-red-700 hover:bg-red-100' },
  {
    ...
    label: '😐',
    title: '보통',
    style: 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100',
  },
...
function SignalRow({
  ...
  const aiIcon = s.aiSentiment === 'positive' ? '👍' : s.aiSentiment === 'negative' ? '👎' : '➖'
    ...
    <tr className="border-b hover:bg-gray-50/50 align-top">
      ...
      <td className="px-4 py-3">
        <div className="flex flex-wrap gap-1">
          ...
          {s.lots.length === 0 && <span className="text-xs text-gray-400 italic">주차장 없음</span>}
    ...
    <tr className="border-b hover:bg-gray-50/50 align-top">
      <td className="px-4 py-3">
        ...
        <p className="text-[11px] text-gray-400">
          {item.poiLat.toFixed(4)}, {item.poiLng.toFixed(4)}
        </p>
      </td>
      <td className="px-4 py-3">
