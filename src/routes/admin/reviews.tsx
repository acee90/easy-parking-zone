      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        {Object.entries(stats.counts)
          ...
          .map(([src, cnt]) => (
            <StatCard
              key={src}
              label={SOURCE_BADGE[src]?.label ?? src}
              value={cnt}
            />
          ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border bg-white">
          <Table>
            ...
            <TableBody>
              {reviews.map((r) => {
                const badge = SOURCE_BADGE[r.source] ?? { label: r.source, class: "bg-gray-50 text-gray-600" };
                return (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-center text-sm">
                      {r.overallScore != null ? r.overallScore : <span className="text-gray-300">-</span>}
                    </TableCell>
